function [nProj, jBasis] = jDynamics(nData, varargin)
%jDYNAMICS Perform j-PCA (dynamic principal component analysis) on input data.
%
%   [nProj, jBasis] = jDynamics(nData, Name, Value) performs jPCA on nData 
%   and returns the projections (nProj) and j-Basis vectors (jBasis). It 
%   also provides visualization options and allows for the inclusion of 
%   additional parameters through name-value pair arguments.
%
%   INPUT =================================================================
%
...
%       The derived j-Basis vectors OF j-PCA.
%
%   FUNCTION OPERATIONS ===================================================
%
...
%   bPlanes       = 0; 
%
%   [nProj, jBasis] = jDynamics(nX, 'numComponents', numComponents,...
%   'bAnimate', bAnimate, 'b3D', b3D, 'bPlanes', bPlanes)
%
%   AUTHOR =================================================================
%
...
% Center data. 
nData = nData - mean(nData,2); 

% Perform PCA on centered data. 
[~, PCs, ~] = pca(nData', 'NumComponents', numComponents, 'Centered', false, 'Algorithm', 'eig');

% Transpose the scores. 
PCs = PCs'; 

% Calculate derivative of the scores.
dPCs = diff(PCs, 1, 2)';
...
Mskew_optimized = optimSkew(Mskew, PCs, dPCs);
